...
export const TenantsView: React.FC = () => {
  ...
  const generateWhatsAppQrCode = async (instanceName: string, renew = false) => {
    setWaLoading(true);
    if (renew) {
      setWaQrCode(null);
    }
            ...
            {selectedTenant.tenant.preferred_channel === 'whatsapp' && (
              <div className="p-4 bg-dark-900 border border-emerald-500/30 rounded-xl space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={() => generateWhatsAppQrCode(`tenant_${selectedTenant.tenant.id}`, true)}
                    disabled={waLoading}
                  ...
                  <div className="flex flex-col items-center space-y-2 pt-2 bg-white/5 p-3 rounded-xl border border-slate-700">
                    <div className="relative inline-block">
                      ...
                      {waExpiresIn <= 0 && (
                        <div className="absolute inset-0 bg-slate-950/85 backdrop-blur-sm rounded-lg flex flex-col items-center justify-center p-3 text-center space-y-2 border border-rose-500/40">
                          ...
                          <span className="text-[11px] font-bold text-white">QR Kodun Vaxtı Bitdi</span>
                          <button
                            type="button"
                            onClick={() => generateWhatsAppQrCode(`tenant_${selectedTenant.tenant.id}`, true)}
                            disabled={waLoading}
                            className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-[10px] font-bold transition flex items-center gap-1 shadow-lg shadow-emerald-500/20"
                          >
                    ...
                    {waExpiresIn > 0 && (
                      <div className="flex items-center gap-1.5 text-[11px] text-amber-300 font-medium">
                        ...
                        <span>Etibarlılıq müddəti: <b className="font-mono">{waExpiresIn}s</b></span>
                      </div>
                    )}
