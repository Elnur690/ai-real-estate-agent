...
class BrochureGeneratorService:
    ...
    async def generate_property_brochure(db: AsyncSession, listing_id: int, tenant_id: int) -> Dict[str, Any]:
        ...
        title_style = ParagraphStyle('BTitle', parent=styles['Heading1'], fontName='AzArial-Bold', fontSize=20, leading=24, textColor=colors.HexColor('#0F172A'), spaceAfter=8)
        sub_style = ParagraphStyle('BSub', parent=styles['Normal'], fontName='AzArial', fontSize=10, leading=14, textColor=colors.HexColor('#475569'), spaceAfter=12)
        body_style = ParagraphStyle('BBody', parent=styles['Normal'], fontName='AzArial', fontSize=10, leading=14, textColor=colors.HexColor('#334155'), spaceAfter=6)

        elements = [
            Paragraph(f"<b>{agent_name}</b> — Eksklüziv Əmlak Broşuru", sub_style),
            Paragraph(listing.title, title_style),
            ...
            Paragraph(f"<b>Məkan:</b> {listing.district or 'Bakı'}", body_style),
            Paragraph(f"<b>Otaq Sayı:</b> {listing.rooms or '-'} otaqlı", body_style),
            Paragraph(f"<b>Sahə:</b> {listing.area_sqm or '-'} m²", body_style),
            Paragraph(f"<b>Təsvir:</b> {listing.description or 'Ətraflı məlumat üçün əlaqə saxlayın.'}", body_style),
            Spacer(1, 15),
            Paragraph(f"<b>Əlaqədar Agent:</b> {agent_name} | <b>Tel:</b> {agent_phone}", title_style)
        ]
