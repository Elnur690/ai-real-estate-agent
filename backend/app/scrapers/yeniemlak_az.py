...
from app.scrapers.utils import get_random_headers, fetch_stealth_page, polite_delay
...
class YeniEmlakAzScraper(BaseScraper):
    ...
    async def fetch_item_details(item_id_or_url: str) -> dict:
        ...
        try:
            res_text, res_status = await fetch_stealth_page(clean_url, headers=headers, timeout=8.0, referer="https://yeniemlak.az/elan/axtar")
    ...
    async def scrape_source(self, url_or_handle: str = "https://yeniemlak.az/elan/axtar") -> List[RawListingItem]:
        ...
        for target_url in urls_to_fetch:
            try:
                ...
                res_text, res_status = await fetch_stealth_page(target_url, headers=headers, timeout=8.0, referer="https://yeniemlak.az/")
                if res_status != 200 or not res_text:
                    continue
                soup = BeautifulSoup(res_text, "html.parser")
