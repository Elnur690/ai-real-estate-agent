...
class BinamAzScraper(BaseScraper):
    ...
    async def scrape_source(self, url_or_handle: str = "https://binam.az/") -> List[RawListingItem]:
        ...
        try:
            ...
            res_text, res_status = await fetch_stealth_page(self.BASE_URL, headers=headers, timeout=15.0, referer="https://binam.az/")
            if res_status == 200 and res_text:
                ...
                for a in links:
                    ...
                    raw_text = parent.get_text(separator=" | ", strip=True).replace('\xa0', ' ')
