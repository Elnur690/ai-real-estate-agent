...
class UnvanAzScraper(BaseScraper):
    async def scrape_source(self, url_or_handle: str = "https://unvan.az/dasinmaz-emlak") -> List[RawListingItem]:
        logger.info(f"[UnvanAzScraper] Fetching listings from {url_or_handle}")
        items: List[RawListingItem] = []

        target_urls = [
            url_or_handle,
            "https://unvan.az/emlak-alqi-satqisi"
        ] if url_or_handle in ["https://unvan.az/", "https://unvan.az/dasinmaz-emlak"] else [url_or_handle]

        seen = set()
        try:
            headers = get_random_headers(referer="https://unvan.az/")
            headers["Accept"] = "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8"
            
            for target_url in target_urls:
                try:
                    await polite_delay(1.2, 2.5)
                    res_text, res_status = await fetch_stealth_page(target_url, headers=headers, timeout=10.0, referer="https://unvan.az/")
                    if res_status == 200 and res_text:
                        ...
                        for a in links:
                            ...
                            parent = a.find_parent("table") or a.find_parent("tr") or (a.parent.parent if a.parent else a)
                            ...
                            if parent:
                                ...
                                if img_el:
                                    ...
                                    if src_val and "http" in src_val:
                                        ...
                                        card_photos.append(f"https://unvan.az{src_val}")

                            items.append(RawListingItem(
                                external_id=f"unvan_{ext_id}",
                                title=title,
                                description=f"Unvan.az elanı: {raw_text[:200]}",
                                price=price,
                                currency="AZN",
                                district=district,
                                metro_station=metro,
                                ...
                                listing_url=f"https://unvan.az{href}" if href.startswith('/') else href,
                                photos=card_photos
                            ))
                            if len(items) >= 30:
                                break
                except Exception as loop_err:
                    logger.debug(f"[UnvanAzScraper] Error fetching {target_url}: {loop_err}")

        except Exception as e:
            logger.info(f"[UnvanAzScraper] Unvan.az scrape status: {e}")

        logger.info(f"[UnvanAzScraper] Extracted {len(items)} listings.")
        return items
