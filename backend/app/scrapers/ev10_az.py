...
class Ev10AzScraper(BaseScraper):
    ...
    async def fetch_item_details(item_id_or_url: str) -> dict:
        """Fetches detailed info like exact price, phone number and full description."""
        ...
        try:
            res_text, res_status = await fetch_stealth_page(url, headers=headers, timeout=8.0, referer="https://ev10.az/")
        ...
        try:
            ...
            for target_url in target_urls:
                try:
                    ...
                    res_text, res_status = await fetch_stealth_page(target_url, headers=headers, timeout=10.0, referer="https://ev10.az/")
                    if res_status != 200 or not res_text:
                        continue
                    soup = BeautifulSoup(res_text, "html.parser")
