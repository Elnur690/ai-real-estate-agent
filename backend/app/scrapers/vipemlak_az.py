...
class VipEmlakAzScraper(BaseScraper):
    async def scrape_source(self, url_or_handle: str = "https://vipemlak.az/") -> List[RawListingItem]:
        ...
        target_urls = [
            url_or_handle,
            "https://vipemlak.az/alqi-satqi/",
            "https://vipemlak.az/kiraye/"
        ] if url_or_handle in ["https://vipemlak.az/", "https://vipemlak.az/elanlar/"] else [url_or_handle]
        ...
        try:
            ...
            for target_url in target_urls:
                try:
                    ...
                    res_text, res_status = await fetch_stealth_page(target_url, headers=headers, timeout=10.0, referer="https://vipemlak.az/")
                    ...
                    logger.debug(f"[VipEmlakAzScraper] Error fetching {target_url}: {loop_err}")

        except Exception as e:
            logger.info(f"[VipEmlakAzScraper] VipEmlak scrape status: {e}")
