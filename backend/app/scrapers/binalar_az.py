...
class BinalarAzScraper(BaseScraper):
    async def scrape_source(self, url_or_handle: str = "https://binalar.az/") -> List[RawListingItem]:
        ...
        try:
            ...
            headers = get_random_headers(referer="https://binalar.az/")
            headers["Accept"] = "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8"
            res_text, res_status = await fetch_stealth_page(url_or_handle, headers=headers, timeout=12.0, referer="https://binalar.az/")
            if res_status == 200 and res_text:
                ...
                for a in links:
                    ...
                    rooms_m = re.search(r'(\d+)\s*-\s*otaql', href) or re.search(r'(\d+)\s*otaq', raw_text)
                    ...
                    if parent:
                        ...
                        if img_el:
                            ...
                            if src_val and "http" in src_val:
                                ...
                                card_photos.append(f"https://binalar.az{src_val}")

                    items.append(RawListingItem(
                        external_id=f"binalar_{ext_id}",
                        title=title,
                        description=f"Binalar.az elanı: {raw_text[:200]}",
                        price=price,
                        currency="AZN",
                        district=district,
                        metro_station=metro,
            ...
            logger.debug(f"[BinalarAzScraper] Source binalar.az temporarily unreachable: {e}")
        except Exception as e:
            logger.warning(f"[BinalarAzScraper] Error scraping: {e}")

        logger.info(f"[BinalarAzScraper] Extracted {len(items)} listings.")
        return items
